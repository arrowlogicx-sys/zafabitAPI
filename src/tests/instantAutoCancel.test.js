...
const { expireUnassignedInstant } = require('../utils/instantDispatch');

describe('Instant Booking Auto-Cancel after Timeout', () => {
  let mongo;
  let customer;
  let service;
  ...
  beforeAll(async () => {
    ...
    customer = await User.create({
      ...
      email: 'instant.customer@zaffabit.test',
      phone: '+919100000101',
  ...
  it('should automatically cancel and transition payment status to refunded if an instant booking remains searching/unassigned', async () => {
    // 1. Create an instant booking that is paid but still searching/unassigned
    const booking = await Booking.create({
      customer: customer._id,
      service: service._id,
      bookingType: 'instant',
      scheduleDate: new Date(),
      dispatchStartedAt: new Date(Date.now() - 10 * 60 * 1000), // 10 mins ago
      status: 'searching',
      paymentStatus: 'paid',
      subtotal: 400,
      platformFee: 15,
    ...
    const payment = await Payment.create({
      ...
      status: 'captured',
      method: 'upi',
      razorpayPaymentId: 'pay_instant_test',
      razorpayOrderId: 'order_instant_test',
    });

    // 3. Trigger the expireUnassignedInstant check
    const result = await expireUnassignedInstant(booking._id, {
    ...
    expect(notification.message).toContain('No maid accepted your instant booking');
  });

  it('should skip cancellation if the instant booking is already accepted/assigned', async () => {
    // 1. Create an instant booking that is already accepted
    const booking = await Booking.create({
      customer: customer._id,
      service: service._id,
      bookingType: 'instant',
      scheduleDate: new Date(),
      dispatchStartedAt: new Date(Date.now() - 10 * 60 * 1000),
      status: 'accepted',
      paymentStatus: 'paid',
      subtotal: 400,
      platformFee: 15,
    ...
    await Payment.create({
      ...
      status: 'captured',
      method: 'upi',
      razorpayPaymentId: 'pay_instant_test2',
      razorpayOrderId: 'order_instant_test2',
    });
